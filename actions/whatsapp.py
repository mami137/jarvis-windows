...
def _auto_send(whatsapp_name: str = "WhatsApp") -> tuple[bool, str]:
    """Enter'a basarak mesajı gönderir. SendKeys (PowerShell) öncelikli;
    pyautogui kurulu değilse SendKeys kullanılır, kuruluysa pyautogui."""

    if not HAS_PYAUTOGUI:
        return _auto_send_with_sendkeys(whatsapp_name)
    ok, detail = _auto_send_with_pyautogui()
    if ok:
        return True, "ok"
    return _auto_send_with_sendkeys(whatsapp_name)
...
def _activate_and_press_enter(whatsapp_name: str) -> bool:
    """
    Açılan WhatsApp penceresini (Desktop, Chrome, Edge veya Firefox) öne alıp
    SendKeys ile Enter'a basar. pyautogui GEREKMEZ; PowerShell ile çalışır.
    """
    candidates = []
    if whatsapp_name:
        candidates.append(whatsapp_name)
    candidates += ["WhatsApp", "chrome", "msedge", "firefox"]
    inner = ",".join(f"'{c}'" for c in candidates)
    try:
        cmd = (
            ...
            "      $_.ProcessName -ilike ('*' + $c + '*') } | Select-Object -First 1; "
            "  if ($p) { if ($ws.AppActivate([int]$p.Id)) { "
...
def _auto_send_with_sendkeys(whatsapp_name: str) -> tuple[bool, str]:
    """PowerShell SendKeys ile odaklanıp Enter'a basar — pyautogui'siz çalışır."""
    try:
        time.sleep(AUTO_SEND_DELAY_SECONDS)
        ok = _activate_and_press_enter(whatsapp_name)
        return (ok, "ok") if ok else (False, "pencere odaklanamadı")
    except Exception as exc:
        return False, str(exc)
    ...
    if app_target in {"auto", "desktop"}:
        if normalized_phone:
            ...
            if ok:
                ...
                ok_send, send_detail = _auto_send(resolved_name or "WhatsApp")
                if ok_send:
